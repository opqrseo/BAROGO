	...
	const res_btn1 =function(e){
		var i =document.createElement('button');
		i.setAttribute('class','reserv_btns1')
		i.innerHTML='예약승인';			
	...
	$.ajax({
		...
		success : function(respond){
			for(let i=0; i<respond.length;i++){
				...
				res_btn1(i+1);
				res_btn2(i+1);
				if(respond[i].confirm=="대기중"){
					document.getElementsByClassName('res_hotel_confirm')[i+1].innerHTML=respond[i].confirm
					document.getElementsByClassName('reserv_btns1')[i].onclick= function(){
						let con=confirm("예약을 확정하시겠습니까?")
						if(con){
				...
				else if(respond[i].confirm=="예약승인"){
					document.getElementsByClassName('res_hotel_confirm')[i+1].innerHTML=respond[i].confirm
					document.getElementsByClassName('reserv_btns1')[i].setAttribute("hidden","hidden");
					document.getElementsByClassName('reserv_btns2')[i].onclick= function(){
						let con=confirm("정말로 취소하시겠습니까?")
						if(con){
							const id= innerHTML=respond[i].reservation_id;
